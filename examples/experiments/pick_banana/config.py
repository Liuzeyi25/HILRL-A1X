...
class EnvConfig(DefaultA1XEnvConfig):
    ...
    IMAGE_CROP = {
        ...
        "side_policy_256": lambda img: img[315:-1, 417:-356],
...
# - -0.09851063829787234
    ...
    # 调整为“更靠右、更靠上”的 reset 起始位：
    # - joint1 略增（偏右）
    # - joint2 / joint3 小幅调整（抬高）
    RESET_JOINT_STATE = np.array([-0.08659574468085106, 1.885106382978722, -1.126595744680851, 0.7657446808510638, -0.147446808510638296, -0.09851063829787234, 70.0])  # 夹爪张开
    ...
    ACTION_SCALE = np.array([0.005, 0.005, 0.005, 0.0, 0.0, 0.0, 0.20]) # [x y z roll pitch yaw gripper]
    # ACTION_SCALE: np.ndarray = np.ones((7,))  # Scaling for joint actions
    
    # 关节限制 (安全范围)
    # 基于 A1_X 的实际关节限制
    JOINT_LIMIT_LOW = np.array([-2.880, -0.001, 0.0,  1.5, 1.521, -1.56, 2.0])
    JOINT_LIMIT_HIGH = np.array([2.880, 3.14, -2.95, -1.55, -1.52,  1.56, 70.0])
...
class TrainConfig(DefaultTrainingConfig):
    ...
    task_desc = "Pick up the banana from the plate"
