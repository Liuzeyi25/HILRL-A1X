...
class EnvConfig(DefaultA1XEnvConfig):
    ...
    TARGET_JOINT_STATE = np.array([0.0, -0.5, 0.0, -1.5, 0.0, 1.0, 20.0])  # 抓取位置
    
    # 重置关节配置 (中立位置)
    RESET_JOINT_STATE = np.array([-0.01531, 1.8255, -1.139, 0.868, -0.053, -0.103, 100.0])  # 夹爪张开
        ...
        if not fake_env:
            if self.teleoperation_device == "gello":
                env = GelloIntervention(
                    ...
                    sync_on_reset=True,           # ✅ 启用 reset 时 Gello 自动跟随机械臂
                    reset_follow_duration=3.0     # ⏱️ Gello 跟随持续时间(秒)
                )
