...
from serl_launcher.networks.reward_classifier_nnx import load_classifier_func
...
class EnvConfig(DefaultEnvConfig):
    ...
    REALSENSE_CAMERAS = {
        "wrist_1": {
            "serial_number": "f0210138", #f0190751
            "dim": (1280, 720),
            # "exposure": 40000,
        },
        "wrist_2": {
    ...
    RESET_Q_POSE= np.array([              # 复位位姿
        0.38, 1.60, -0.92, -1.66, 0.65, 1.96
    ])
    RESET_POSE = np.array([0.30374736, 0.11237716, 0.22896096, np.pi, 0, 0.5*np.pi]) # (w x y z)
