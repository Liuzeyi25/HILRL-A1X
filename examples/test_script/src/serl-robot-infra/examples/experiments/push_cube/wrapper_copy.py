...
class CubeEnv(FakeEnv):
    def __init__(self, **kwargs):
        ...
        listener = keyboard.Listener(
            on_press=on_press)
        listener.start()
        quat = euler_2_quat(self.config.RESET_POSE[3:])  # 返回 [x, y, z, w]
        pos = self.config.RESET_POSE[:3]     
        self.action_scale = self.config.ACTION_SCALE# [x, y, z]
        self.resetpos = np.concatenate([pos, quat])        # [pos],  [qx, qy, qz] w,
    ...
    def go_to_reset(self, joint_reset=False):
        ...
        self._send_joint_command(self.curr_q)
        time.sleep(0.3)
        
        requests.post(self.server + "update_param", json=self.config.PRECISION_PARAM)

        # pull up
        self._update_currpos()
        # reset_pose = copy.deepcopy(self.currpos)
        # reset_pose[2] = self.resetqpos[2] + 0.04
        self._send_joint_command(self.resetqpos)
        print("Resetting to joint position:", self.resetqpos)

        # perform joint reset if needed
        if joint_reset:
            print("JOINT RESET")
            requests.post(self.server + "jointreset")
            time.sleep(0.5)

        # perform Cartesian reset
        if self.randomreset:  # randomize reset position in xy plane
            print("RANDOM RESET", self.randomreset)
            # breakpoint()
        ...
        requests.post(self.server + "update_param", json=self.config.COMPLIANCE_PARAM)
    ...
    #     requests.post(self.server + "close_gripper_slow")
