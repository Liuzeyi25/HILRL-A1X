...
class JoystickExpert:
    def __init__(self, deadband=0.2, scale_xyz=0.5, scale_rpy=0.3):
        pygame.init()
        pygame.joystick.init()

        if pygame.joystick.get_count() == 0:
            raise Exception("❌ 没有检测到任何手柄设备！")
        else:
        ...
        self.scale_rpy = scale_rpy

    def get_axis_with_deadband(self, axis_index):
        # print("Now axis_index:", axis_index)
        #breakpoint()
        val = self.joystick.get_axis(axis_index)
        return val if abs(val) >= self.deadband else 0.0

    def get_action(self):
        pygame.event.pump()

        # ✅ 位姿控制轴映射（参考你的ROS代码）：
        # axes[1] → x, axes[0] → y, axes[4] → z
        # axes[3] → roll, axes[6] → pitch, axes[7] → yaw
        dx = self.get_axis_with_deadband(1) * self.scale_xyz  # 前后
        dy = self.get_axis_with_deadband(0) * self.scale_xyz  # 左右
        dz = - self.get_axis_with_deadband(4) * self.scale_xyz  # 上下

        droll  = self.get_axis_with_deadband(3) * self.scale_rpy
        # dpitch = self.get_axis_with_deadband(6) * self.scale_rpy
        # dyaw   = self.get_axis_with_deadband(7) * self.scale_rpy  # 可选
        dpitch = 0.3 if self.joystick.get_button(2) else -0.3 if self.joystick.get_button(1) else 0.0

        dyaw = 0.0

        # ✅ 夹爪控制（按钮）
        # buttons[4] → 打开，buttons[5] → 闭合
        reset = self.joystick.get_button(0)
        open_pressed = self.joystick.get_button(4)
        close_pressed = self.joystick.get_button(5)
        buttons = (close_pressed, open_pressed)  # left=close, right=open
        # print(f"Joystick action: dx={dx}, dy={dy}, dz={dz}, droll={droll}, buttons={buttons}")
        # 返回 xyz + rpy + 按钮状态
        return np.array([dx, dy, dz, droll, dpitch, dyaw]), buttons
...
class JoystickIntervention(gym.ActionWrapper):
    ...
    def action(self, action: np.ndarray) -> np.ndarray:
        ...
        if np.linalg.norm(expert_a) > 0.01:
            print("Joystick intervention:", expert_a)
            intervened = True
