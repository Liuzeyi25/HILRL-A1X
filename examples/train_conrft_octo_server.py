            ...
            if episode == 0:
                ...
                for key in obs.keys():
                    if isinstance(obs[key], dict):
                        print(f"  {key} (dict): {list(obs[key].keys())}")
...
def learner(rng, tasks, agent, replay_buffer, demo_buffer, wandb_logger=None):
    ...
    latest_ckpt = checkpoints.latest_checkpoint(FLAGS.checkpoint_path) if FLAGS.checkpoint_path else None
    start_step = (
    ...
    if step < FLAGS.pretrain_steps:
        ...
        print(f"   - 目标步数: {FLAGS.pretrain_steps}")
        print(f"   - Batch size: {config.batch_size}")
        print(f"   - CTA ratio: {config.cta_ratio}")
        print(f"   - Demo buffer 大小: {len(demo_buffer)}")
        print(f"   - 使用设备: {[str(d) for d in jax.devices()]}")
        print(f"   - Sharding: replicate on {len(jax.devices())} devices")
        print()
...
def main(_):
    ...
    if FLAGS.checkpoint_path is None:
        ...
        print(f"⚠️ Warning: --checkpoint_path not set. Using default: {FLAGS.checkpoint_path}")
    ...
    env = config.get_environment(
        ...
        classifier=False, 
        stack_obs_num=2,
        eval_mode=is_eval_mode,  # 🎯 传递评估模式标志
    )
    ...
    print("🔄 正在将 agent 复制到所有设备...")
    agent = jax.device_put(jax.tree_util.tree_map(
        jnp.array, agent), sharding.replicate())
    print(f"✅ Agent 已复制到 {num_devices} 个设备")

    # 🎯 评估模式下,在 actor() 函数中加载指定的 checkpoint,这里跳过
    if FLAGS.checkpoint_path is not None and os.path.exists(FLAGS.checkpoint_path) and not is_eval_mode:
        ...
        if latest_ckpt is not None:
            ...
            print_green("No previous checkpoint found. Starting from scratch.")
