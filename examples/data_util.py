...
def add_embeddings_to_trajectory(trajectory, model, tasks):
    """
    update every transition in the trajectory and add embeddings
    return the updated trajectory
    """
    for i in range(len(trajectory)):
        observation = trajectory[i]['observations']

        # 灵活处理相机键名 - 优先使用 side_policy_256，如果不存在则使用 wrist_1
        if "side_policy_256" in observation:
            image_primary = observation["side_policy_256"]
        elif "wrist_1" in observation:
            image_primary = observation["wrist_1"]
        else:
            raise KeyError("Neither 'side_policy_256' nor 'wrist_1' found in observations")
        
        # 获取腕部相机图像
        if "wrist_1" in observation:
            image_wrist = observation["wrist_1"]
        else:
            # 如果没有腕部相机，使用主相机作为备用
            image_wrist = image_primary
