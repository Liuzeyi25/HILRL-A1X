...
########################################################################
# define for Proxy RL
flags.DEFINE_string("VLM_model_dir", '/home/dungeon_master/conrft/examples/experiments/insert_block/checkpoints_vlm', "Path to the Qwen-VL-RewardModel checkpoint directory for VLM-based reward shaping. Set to 'none' to disable VLM reward.")
flags.DEFINE_boolean("use_vlm_reward", True, "Whether to use VLM-based reward shaping.")
flags.DEFINE_boolean("use_vlm_proxy", False, "Whether to use VLM as a proxy reward model to relabel rewards in the replay buffer for offline RL training. If True, it will relabel rewards for all transitions in the episode based on the VLM output at the end of the episode, and these relabeled rewards will be used for training the agent.")
...
def create_temp_videos_from_transitions(transitions):
    """
    从暂存的 transition 列表中提取双视角图像并保存为临时 mp4 文件。
    返回 main_video_path, wrist_video_path
    WARNING: 这个函数会在磁盘上创建临时视频文件，调用者需要负责删除这些文件以避免磁盘空间泄漏。
    WARNING: 目前不用wrist视频了，所以注释掉相关代码，等需要再放开
    """
    main_path = tempfile.mktemp(suffix='_main.mp4')
    # wrist_path = tempfile.mktemp(suffix='_wrist.mp4')
    
    # 假设你的相机 key 是 'side_policy_256' 和 'wrist_1'
    main_frames = [t["observations"]["side_policy_256"] for t in transitions]
    # wrist_frames = [t["observations"]["wrist_1"] for t in transitions]
    
    # 处理帧堆叠 (Frame Stacking)
    if len(main_frames[0].shape) == 4:
        main_frames = [f[-1] for f in main_frames]
        # wrist_frames = [f[-1] for f in wrist_frames]
    ...
    write_video(main_frames, main_path)
    # write_video(wrist_frames, wrist_path)
    
    # return main_path, wrist_path
    return main_path  # wrist 视频暂时不使用
...
def actor(agent, data_store, intvn_data_store, env, sampling_rng):
    ...
    #########################################################
    if FLAGS.use_vlm_reward or FLAGS.use_vlm_proxy:
        vlm = VLMProxy(FLAGS.VLM_model_dir)
        TASK_INSTRUCTION ="insert a cube into the corresponding hole on the board."

    # 引入我们用来暂存 Episode 数据以便后续处理的列表
    episode_buffer = [] 
    ...
    for step in pbar:
        ...
        with timer.context("step_env"):
            ...
            transition = dict(
                ...
                _is_intvn=already_intervened,  # 方便后续基于 _is_intvn 进行过滤，而不依赖 label 的具体数值
            )
            if 'grasp_penalty' in info:
                transition['grasp_penalty']= info['grasp_penalty']

            #####################################################
            # append transition to episode buffer for VLM processing
            # 暂时先不insert到 data_store，等 episode 结束后统一处理视频和 VLM reward，再批量插入
            ...
            #####################################################
                # episode 结束，处理视频和 VLM reward
                if FLAGS.use_vlm_reward:
                    with timer.context("vlm_reward_shaping"):
                        print(f"\n[Episode End] 轨迹长度: {len(episode_buffer)} 步。正在合成视频...")
                        main_vid = create_temp_videos_from_transitions(episode_buffer)
                        print("正在调用 Qwen3-VL 代理分配 Dense Reward...")
                        try:
                            step_rewards = vlm.get_trajectory_reward(
                                main_vid, TASK_INSTRUCTION, len(episode_buffer)
                            )
                            for step_idx, added_reward in step_rewards.items():
                                episode_buffer[step_idx]["rewards"] = added_reward

                        except Exception as e:
                            print(f"[VLM 错误] Reward 标注失败，使用环境原始稀疏奖励。错误信息: {e}")

                        
                        # 清理临时视频文件
                        if os.path.exists(main_vid): os.remove(main_vid)
                        # if os.path.exists(wrist_vid): os.remove(wrist_vid)

                # --- 4. 批量插入 Data Store，供 Learner 训练 ---
                for t in episode_buffer:
                    is_intvn = t.pop('_is_intvn', False) # 弹出临时标记
                    data_store.insert(t)
                    if is_intvn:
                ...
                if config.buffer_period > 0 and FLAGS.checkpoint_path:
                    # -------------------------------------------
                    # 此时的 episode_buffer 已经包含了 VLM 增强后的 reward
                    if episode_buffer:
                        with open(os.path.join(buffer_path, f"transitions_{step}.pkl"), "wb") as f:
                            pkl.dump(episode_buffer, f)
                    # -------------------------------------------
...
def main(_):
    ...
    if FLAGS.checkpoint_path is not None and os.path.exists(FLAGS.checkpoint_path):
        ...
        agent = agent.replace(state=ckpt)
        # print(os.path.abspath(FLAGS.checkpoint_path))
