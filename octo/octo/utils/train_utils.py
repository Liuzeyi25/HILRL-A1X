...
def batched_apply(fn, batch_size):
    ...
    def wrapped_fn(*args, **kwargs):
        ...
        for i in range(0, input_batch_size, batch_size):
            ...
            step_args, step_kwargs = jax.tree_util.tree_map(
        ...
        return jax.tree_util.tree_map(lambda *args: np.concatenate(args, axis=0), *outputs)
...
def freeze_weights(
    ...
    total_params = sum(
        jax.tree_util.tree_leaves(
            jax.tree_util.tree_map(lambda x: x.size, params_or_params_shape)
        )
    )
...
def create_optimizer(
    ...
    if frozen_keys:
        ...
        zero_frozen_params = lambda params: jax.tree_util.tree_map(
