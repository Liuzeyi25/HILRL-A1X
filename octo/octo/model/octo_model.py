...
class OctoModel:
    ...
    def sample_transformer(
        ...
        timestep_pad_mask: Optional[ArrayLike] = None,
        train: bool = False,
        sample_shape: Tuple[int, ...] = (),##贴合jax0.6.2版本的改动而增加
        argmax: bool = False,              ##贴合jax0.6.2版本的改动而增加
    ):
        ...
        Args:
            ...
            timestep_pad_mask: (batch_size, window_size) Boolean mask that is False when the timestep corresponds to padding
            train: whether to run in train mode
            sample_shape: shape for sampling (unused, kept for API compatibility)
            argmax: whether to use argmax (unused, kept for API compatibility)
            ...see `action_heads.py` for the rest of the kwargs.
        Returns:
    ...
    def load_pretrained(
        ...
        logging.debug(
            ...
            flax.core.pretty_repr(jax.tree_util.tree_map(
        ...
        if jax.process_index() == 0:
            ...
            if not tf.io.gfile.exists(dataset_statistics_path):
                with tf.io.gfile.GFile(dataset_statistics_path, "w") as f:
                    json.dump(
                        jax.tree_util.tree_map(lambda x: x.tolist(),
    ...
    def get_pretty_spec(self):
        ...
        if self.text_processor is not None:
            task_space["language_instruction"] = jax.tree_util.tree_map(
