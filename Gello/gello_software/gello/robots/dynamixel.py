...
class DynamixelRobot(Robot):
    ...
    def command_joint_state(self, joint_state: np.ndarray) -> None:
        # This must be the exact inverse of get_joint_state()
        # get_joint_state does:
        #   1. pos = (driver.get - offsets) * signs
        #   2. pos[-1] = normalize_gripper(pos[-1])  # if gripper exists
        # So command_joint_state must do:
        #   1. pos[-1] = denormalize_gripper(pos[-1])  # if gripper exists
        #   2. driver.set = pos * signs + offsets
        
        joint_state = joint_state.copy()  # Don't modify input
        
        # Handle gripper denormalization
        if self.gripper_open_close is not None:
            # Convert gripper from [0, 1] back to raw angle
            g_normalized = joint_state[-1]
            g_raw = g_normalized * (self.gripper_open_close[1] - self.gripper_open_close[0]) + self.gripper_open_close[0]
            joint_state[-1] = g_raw
        
        # Inverse transformation: driver.set = pos * signs + offsets
        # Since signs are ±1, multiplication handles direction reversal
