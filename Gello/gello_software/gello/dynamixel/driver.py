...
class DynamixelDriver(DynamixelDriverProtocol):
    ...
    def set_joints(self, joint_angles: Sequence[float], use_individual_writes: bool = True):
        if len(joint_angles) != len(self._ids):
            raise ValueError(
                "The length of joint_angles must match the number of servos"
            )
        ...
        # Use individual writes to avoid sync write issues with position control mode
        if use_individual_writes:
            with self._lock:  # Critical: lock to avoid conflicts with reading thread
        ...
        # Original GroupSyncWrite method (fallback)
        for dxl_id, angle in zip(self._ids, joint_angles):
            # Convert the angle to the appropriate value for the servo
            position_value = int(angle * 2048 / np.pi)
            
            print(f"[Driver] ID {dxl_id}: angle={angle:.4f} rad → position={position_value}")
            ...
            if not dxl_addparam_result:
                print(f"[Driver] addParam FAILED for ID {dxl_id}")
                raise RuntimeError(
        ...
        if dxl_comm_result != COMM_SUCCESS:
            print(f"[Driver] txPacket FAILED: result={dxl_comm_result}")
