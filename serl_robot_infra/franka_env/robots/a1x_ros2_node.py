...
class A1XRobotZMQNode(Node):
            ...
            try:
                ...
                if self.command_socket.poll(timeout=1):
                    ...
                    if cmd == "command_joint_state":
                        ...
                        if ret is None:
                            self.command_socket.send_json({"status": "error", "error": "invalid eef_pose"})
                        else:
                            if async_mode:
                                # Async mode: send ack immediately
                                self.command_socket.send_json({"status": "ok", "async": True})
                            else:
                                ...
                                if reached:
                                    self.command_socket.send_json({"status": "ok", "info": info})
                                else:
                                    self.command_socket.send_json({"status": "timeout", "info": info})
    ...
    def publish_eef_command(self, eef_pose):
        """Publish end-effector pose command to ROS2.
        ...
        with self._lock:
            if self._current_pos is None or self._current_rot is None:
                self.get_logger().warning("Current EE pose not available yet")
                return
            
            current_pos = self._current_pos.copy()
            current_rot = self._current_rot.copy()
        
        print(f"Current EE Pos: {current_pos}, Rot (quat): {current_rot}")

        # Extract delta pose and gripper
        delta_pos = np.array(eef_pose[:3])  # [dx, dy, dz]
        delta_rot_euler = np.array(eef_pose[3:6])  # [drx, dry, drz] in radians
        ...
        # Method 1: Proper rotation composition (recommended for large rotations)
        # Convert current quaternion to rotation object
        current_rotation = R.from_quat(current_rot)  # expects [x, y, z, w]
        
        # Convert delta euler angles to rotation
        delta_rotation = R.from_euler('xyz', delta_rot_euler)
        
        # Apply delta rotation: new_rot = delta_rot * current_rot
        # This applies delta rotation in the CURRENT frame
        new_rotation = delta_rotation * current_rotation
        
        # Method 2: Simple euler angle addition (only for small angles, ~<10 degrees)
        # Uncomment if you prefer euler angle addition for small incremental movements:
        # current_euler = current_rotation.as_euler('xyz')
        # new_euler = current_euler + delta_rot_euler
        ...
        pose_msg.pose.position.x = float(new_pos[0])
        pose_msg.pose.position.y = float(new_pos[1])
        pose_msg.pose.position.z = float(new_pos[2])
        pose_msg.pose.orientation.x = float(new_quat[0])
        pose_msg.pose.orientation.y = float(new_quat[1])
        pose_msg.pose.orientation.z = float(new_quat[2])
        pose_msg.pose.orientation.w = float(new_quat[3])
        
        # Publish pose command
        self.pose_command_pub.publish(pose_msg)
        
        # Log to help debug if commands are actually being sent
        self.get_logger().info(
            f"Published EEF command: pos=[{new_pos[0]:.4f}, {new_pos[1]:.4f}, {new_pos[2]:.4f}], "
            f"quat=[{new_quat[0]:.3f}, {new_quat[1]:.3f}, {new_quat[2]:.3f}, {new_quat[3]:.3f}]"
        )
        
        # Check if anyone is subscribed to our pose topic
        num_connections = self.pose_command_pub.get_subscription_count()
        if num_connections == 0:
            self.get_logger().warn(
                "⚠️  No subscribers to /motion_target/target_pose_arm! "
                "EEF commands may not be executed. "
                "Check if A1_X controller supports Cartesian control."
            )
        
        # Publish gripper command separately
        gripper_msg = Float32()
        gripper_msg.data = float(gripper_position)
        self.gripper_command_pub.publish(gripper_msg)
        
        # Return the intended absolute EE pose (position, quaternion)
        # new_pos: numpy array (3,), new_quat: numpy array (4,) in [x,y,z,w]
